#!/usr/bin/env python3
"""
Eş merkezli çember + yönelim (F) test paterni üreteci.

DÜZENEK
-------
Patern ÜRETEN cihaz (projektör / patern jeneratörü):
    1280 x 1024 px, piksel 13.62 um, açısal çözünürlük 0.027 derece/px
    -> ima edilen odak uzaklığı f = 28.90 mm (pitch / tan(0.027 derece))

Görüntü ALAN cihaz (test edilen):
    f = 47.7 mm, f/1.4, giriş pupili 34 mm, FOV 21.5 derece (+-10.75)

KRİTİK GEOMETRİ
---------------
Projektör paneli cihazın FOV'undan GENİŞ:
    panel dikey    +-13.57 derece   (cihaz +-10.75'i taşıyor)
    panel yatay    +-16.78 derece   (taşıyor)
    panel köşegen  +-21.12 derece   (taşıyor)
Yani cihaz, panelin yalnızca merkezî r = 403 px yarıçaplı dairesini görür.
Panel köşesi 820 px'tedir -> KÖŞEYE KONAN F'LER GÖRÜNTÜYE GİRMEZ.
Bu yüzden F'ler panel köşesine değil, FOV kenarının hemen içine
(r = f_radius, varsayılan 360 px) 45/135/225/315 derece azimutlarına konur.

Yarıçap modeli: r = f * tan(theta) / pitch  (merkezi izdüşüm, tan tabanlı)

Kullanım:
    python generate_circle_pattern.py
    python generate_circle_pattern.py --step 2.0 --f-radius 380
"""
...
# --- Patern üreten cihaz (projektör) ---
W, H = 1280, 1024
PIXEL_PITCH_MM = 13.62e-3               # 13.62 um
ANG_RES_DEG = 0.027                     # derece / px (verilen spec)
FOCAL_PROJ_MM = PIXEL_PITCH_MM / np.tan(np.deg2rad(ANG_RES_DEG))   # ~28.90 mm
CX, CY = (W - 1) / 2.0, (H - 1) / 2.0   # 639.5, 511.5 -- gerçek merkez
...
def main():
    ...
    ap.add_argument("--max-angle", type=float, default=DEV_HALF_FOV,
                    help="en dış çemberin açısı (derece); varsayılan = cihaz yarı-FOV")
    ...
    ap.add_argument("--out", default="circle_pattern.png")
    args = ap.parse_args()

    r_panel = min(CX, CY) - 10.0            # panelin kısa ekseni içinde kalan sınır
    ...
    r_fov = ang_to_px(DEV_HALF_FOV)
    print(f"PROJEKTÖR: {W}x{H}, piksel {PIXEL_PITCH_MM * 1e3:.2f} um, "
          f"{ANG_RES_DEG} derece/px -> f = {FOCAL_PROJ_MM:.2f} mm")
